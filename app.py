...
# ===================== 탭1: AI 챗봇 =====================
with tab1:
    language = st.radio("언어", ["한국어", "English"], horizontal=True, label_visibility="collapsed")
...
with tab2:
    ...
    if weather:
        st.markdown(f"""
        <div class="widget">
            <div class="w-title"><span class="dot-blue"></span>실시간 날씨 · 독도</div>
            <div style="display:flex; align-items:center; gap:14px; margin-bottom:10px;">
                <div class="weather-temp">{weather['temp']}°</div>
                <div class="weather-info">현재 기온<br>독도 · 기상청 연동</div>
            </div>
            <div>
                <span class="wtag">💧 습도 {weather['humidity']}%</span>
                <span class="wtag">💨 풍속 {weather['wind']}m/s</span>
                <span class="wtag">🌧️ 강수 {weather['rain']}mm</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
    else:
...
with tab3:
    ...
    * { margin:0; padding:0; }
    ...
    window.addEventListener('load', function() {
        ...
        L.marker([37.2408, 131.8695], {icon: red}).addTo(map).bindPopup('<b>🏝️ 동도</b>').openPopup();
        L.marker([37.2418, 131.8652], {icon: blue}).addTo(map).bindPopup('<b>🏝️ 서도</b>');
        L.marker([37.2410, 131.8698], {icon: blue}).addTo(map).bindPopup('<b>🔦 동도 등대</b>');
        L.marker([37.2420, 131.8680], {icon: red}).addTo(map).bindPopup('<b>⚓ 동도 선착장</b>');
        L.marker([37.2422, 131.8648], {icon: blue}).addTo(map).bindPopup('<b>🏠 서도 주민숙소</b>');
    });
    </script></body></html>
    """
...
with tab4:
    ...
    with island_col:
        island = st.radio("섬 선택", ["동도", "서도"], horizontal=True, label_visibility="collapsed")
    with season_col:
        season = st.radio("계절", ["봄", "여름", "가을", "겨울"], horizontal=True, label_visibility="collapsed")
    ...
    def get_caption(island, season):
        if island == "동도" and season == "겨울":
            return "하늘에서 바라본 동도"
        elif island == "동도":
            return "서도 정상에서 바라본 동도"
        else:
            return "동도 정상에서 바라본 서도"

    if os.path.exists(img_path):
        st.image(img_path, caption=f"{get_caption(island, season)} ({season})", use_container_width=True)
    else:
        st.warning("해당 사진이 없습니다.")
